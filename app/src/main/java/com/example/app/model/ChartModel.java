 ...
 * Class: ChartModel
 * Description: 图形数据业务实现类
 * Author: zhaoyangzhou
 * Email: zhaoyangzhou@126.com
 * Created on: 2017/12/22 9:44
 */
public class ChartModel implements ChartIA {
    protected List<IStickEntity> ohlc;

    public Observable<List<DateValueEntity>> getData() {
        return Observable.create(new ObservableOnSubscribe<List<DateValueEntity>>() {
            @Override
            public void subscribe(ObservableEmitter<List<DateValueEntity>> emitter) {
                initOHLC();
                List<DateValueEntity> list = initMA(5);
                if (list == null) {
                    emitter.onError(new Exception("list = null"));
                } else {
                    emitter.onNext(list);
                    emitter.onComplete();
                }
